...
echo ffmpeg -framerate 60 -f x11grab -loglevel quiet -s ${width}x${height} -i ${DISPLAY}+$x,$y \
    -f pulse -ac 2 -i default $filename
# run the ffmpeg command
ffmpeg -framerate 60 -f x11grab -loglevel quiet -s ${width}x${height} -i ${DISPLAY}+$x,$y \
    -f pulse -ac 2 -i default $filename
