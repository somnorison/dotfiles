...
function M.config()
  local execs = {
    { nil, "<leader>;h", "Horizontal Terminal", "horizontal", 0.3 },
    { nil, "<leader>;v", "Vertical Terminal", "vertical", 0.4 },
  ...
  -- This function is called on each of the tables in the execs table 
  local add_exec = function(opts)
    ...
    -- whichkey.register({
    --   [opts.keymap] = { "<cmd>lua require('toggleterm').exec_toggle { cmd = '" .. opts.cmd .. "', count = " .. opts.count .. ", direction = '" .. opts.direction .. "', size = " .. opts.size() .. " }<cr>", opts.label },
    -- })
  end
